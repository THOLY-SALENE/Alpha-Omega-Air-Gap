...
"""
🜂 Alpha Omega Air Gap 🜄 (AOAG) — v13.2 REFINED FULL STACK
...
class AOAG:
    ...
    def _refine_nine_hearts(
        ...
        ai_psych = "ai_psychosis_risk_pattern" in labels

        scores[NineHeart.GOLD] = self._clamp(
            stage1.get(NineHeart.GOLD, 0.90) + 0.04 - 0.55 * g - 0.45 * d - 0.25 * elevated - 0.18 * a
            - 0.15 * (1.0 if monet and not has_ev else 0.0)
            - 0.12 * (1.0 if ai_psych else 0.0)
            + 0.10 * (1.0 if has_ev else 0.0)
        )
        scores[NineHeart.GREEN] = self._clamp(
            stage1.get(NineHeart.GREEN, 0.80) + 0.08 + 0.35 * f_pos - 0.55 * instability - 0.28 * f_neg - 0.15 * sem - 0.10 * a
        )
        scores[NineHeart.WHITE] = self._clamp(
            stage1.get(NineHeart.WHITE, 0.88) + 0.05 - 0.32 * fiction - 0.28 * sem - 0.22 * emb - 0.25 * elevated * (0.0 if has_ev else 1.0) + 0.15 * safe_sim
        )
        scores[NineHeart.BLACK] = self._clamp(
            stage1.get(NineHeart.BLACK, 0.64) + 0.20 * proto + 0.16 * sem + 0.14 * emb + 0.12 * (1.0 if (monet or ai_psych) else 0.0)
        )
        scores[NineHeart.RED] = self._clamp(
            stage1.get(NineHeart.RED, 0.84) + 0.04 - 0.38 * injection - 0.30 * destab - 0.12 * sem + 0.10 * (1.0 if has_ev else 0.0)
        )
        scores[NineHeart.ORANGE] = self._clamp(
            stage1.get(NineHeart.ORANGE, 0.70) + 0.14 * safe_sim - 0.20 * a - 0.18 * (1.0 if ai_psych else 0.0) - 0.14 * instability
        )
        scores[NineHeart.YELLOW] = self._clamp(
            stage1.get(NineHeart.YELLOW, 0.82) - 0.18 * a - 0.10 * sem - 0.08 * instability + 0.08 * f_pos
        )
        scores[NineHeart.BLUE] = self._clamp(
            stage1.get(NineHeart.BLUE, 0.84) - 0.18 * emb - 0.14 * fiction - 0.10 * elevated + 0.10 * (1.0 if has_ev else 0.0) + 0.06 * f_pos
        )
        scores[NineHeart.PURPLE] = self._clamp(
            stage1.get(NineHeart.PURPLE, 0.82) + 0.10 * safe_sim - 0.08 * fiction - 0.10 * instability - 0.08 * (1.0 if ai_psych else 0.0)
        )
    ...
    def _calculate_consensus(self, heart_scores: Dict[NineHeart, float], probe_results: List[Dict[str, Any]], claim_tier: ClaimTier, context: Dict[str, Any]) -> Dict[str, Any]:
        weights = {
        ...
        vetoes = []
        if heart_scores[NineHeart.GOLD] < self.config.HEART_GOLD_VETO:
            vetoes.append("gold_veto")
        if heart_scores[NineHeart.GREEN] < self.config.HEART_GREEN_VETO:
            vetoes.append("green_veto")
        if claim_tier.value >= ClaimTier.EXTERNAL_CLAIM.value and heart_scores[NineHeart.WHITE] < self.config.HEART_WHITE_ELEVATED_VETO:
    ...
    def _run_sync_impl(self, input_text: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        consensus_data = self._calculate_consensus(heart_scores, probes, claim_tier=claim_tier, context=context)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Alpha Omega Air Gap (AOAG) CLI test runner. Key thresholds can also be overridden via AOAG_* environment variables.")
